...
internal sealed class SefariaShulchanArukhService : IDisposable
{
    ...
    [
        new("Shulchan Arukh, Orach Chayim", "אורח חיים", 1),
        new("Shulchan Arukh, Yoreh De'ah", "יורה דעה", 2),
        new("Shulchan Arukh, Even HaEzer", "אבן העזר", 3),
        new("Shulchan Arukh, Choshen Mishpat", "חושן משפט", 4)
    ];
    ...
    public SefariaShulchanArukhService(string cacheDirectory)
    {
    ...
    public HalakhicSectionShape GetSectionShape(string englishTitle)
    {
        ...
        using var document = GetCachedJsonDocument(
            $"shulchan-arukh-shape-{SanitizeCacheKey(englishTitle)}",
            $"api/shape/{Uri.EscapeDataString(englishTitle)}",
            ShapeCacheDuration);

        if (!TryExtractSectionShape(document.RootElement, englishTitle, out var shape))
        {
            throw new InvalidOperationException($"לא ניתן היה לפענח את מבנה השולחן ערוך {englishTitle} מ-Sefaria.");
        }
    ...
    public string GetPreferredSectionHebrewTitle(string englishTitle)
    {
        return Sections.FirstOrDefault(section =>
            ...
            ?.HebrewTitle
            ?? englishTitle.Replace("Shulchan Arukh, ", string.Empty, StringComparison.Ordinal);
    }
    ...
    private JsonDocument GetCachedJsonDocument(string cacheKey, string relativeUrl, TimeSpan duration)
    {
        ...
        catch (Exception fetchException)
        {
            ...
            throw new InvalidOperationException("לא ניתן לטעון כעת את נתוני השולחן ערוך מ-Sefaria.", fetchException);
        }
    }
    ...
    private static string CleanSectionHebrewTitle(string? value)
    {
        ...
        return title.StartsWith("שולחן ערוך, ", StringComparison.Ordinal)
            ? title["שולחן ערוך, ".Length..]
            : title;
    }
